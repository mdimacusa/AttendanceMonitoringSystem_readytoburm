...
Public Class AttendanceRecord
    ...
    Sub loadDepartment()
        Try
            con.Close()
            con.Open()
            Dim Cmd As New SqlClient.SqlCommand("SELECT DISTINCT Department FROM tblAttendance", con)
            Cmd.ExecuteNonQuery()
    ...
    Sub loadYearLevel()
        Try
            con.Close()
            con.Open()
            Dim cmd As New SqlClient.SqlCommand("SELECT DISTINCT Yearlevel FROM tblAttendance where Department = '" + cbDepartment.Text + "'", con)
    ...
    '        con.Close()
    '        con.Open()
    '        Dim Cmd As New SqlClient.SqlCommand("SELECT DISTINCT Yearlevel FROM tblAttendance", con)
    '        Cmd.ExecuteNonQuery()
    '        Dim Datatable As New DataTable
    '        Dim DataAdapter As New SqlClient.SqlDataAdapter(Cmd)
    '        DataAdapter.Fill(Datatable)
    '        cbYearLevel.DataSource = Datatable
    '        cbYearLevel.ValueMember = "Yearlevel"
    '        cbYearLevel.DisplayMember = "Yearlevel"
    '    Catch ex As Exception
    '        msgerror(Err.Description)
    '        con.Close()
    '    End Try
    ...
    Sub loadCourse()
        ...
        Try
            con.Close()
            con.Open()
            Dim Cmd As New SqlClient.SqlCommand("SELECT DISTINCT Course FROM tblAttendance", con)
            Cmd.ExecuteNonQuery()
            Dim Datatable As New DataTable
            Dim DataAdapter As New SqlClient.SqlDataAdapter(Cmd)
            DataAdapter.Fill(Datatable)
    ...
    Sub list()
        Try
            ...
            cmd.Connection = con
            'cmd.CommandText = "SELECT id,SubID,StudentID,Fullname,YearLevel,Course,LogIN,LogOUT,SMS,NoStudent,SMS1,Date FROM tblAttendance"
    ...
    Sub ViewReport()
        Try
            ...
            comm.CommandText = "Select * from tblAttendanceCrystalReport"
            comm.CommandType = CommandType.Text
            comm.Connection = con
            da = New SqlDataAdapter(comm)
            da.Fill(ds)
            da.Dispose()
            Dim report As New AttendanceRecordReport
            report.SetDataSource(ds.Tables(0))
            ' ds.WriteXmlSchema("C:\xml\AttendanceRecortCrystalReport.xml")
            ...
            AttendanceRecordViewer.CrystalReportViewer1.ReportSource = report
            AuditTrail("Attendance Record", "Print", "#####-############-#####")
            AttendanceRecordViewer.ShowDialog()
            Deleteall()
        Catch ex As Exception
            msgerror(Err.Description)
        End Try
    ...
    Sub Deleteall()
        Try
            con.Close()
            con.Open()
            cmd.CommandText = "DELETE tblAttendanceCrystalReport"
    ...
    Private Function insertsales()
        ...
        Try
            For Each x As ListViewItem In ListView1.Items
                connect()
                cmd.Connection = con
                cmd.CommandText = "INSERT INTO tblAttendanceCrystalReport (SubID,StudentID,Firstname,Lastname,YearLevel,Course,LogIN,LogOUT,SMS,NoStudent,SMS1,Date)" & _
                "VALUES (@SUBID,@SI,@FN,@LN,@YL,@C,@LI,@LO,@SMS,@NS,@SMS1,@D)"
                ...
                cmd.Parameters.AddWithValue("@SMS", x.SubItems(9).Text)
                cmd.Parameters.AddWithValue("@NS", x.SubItems(10).Text)
                cmd.Parameters.AddWithValue("@SMS1", x.SubItems(11).Text)
                cmd.Parameters.AddWithValue("@D", x.SubItems(12).Text)
    ...
    Sub SortDate()
        Try
            ...
            ListView1.Items.Clear()
            cmd.Connection = con
            cmd.CommandText = "SELECT * FROM tblAttendance WHERE Date >= '" & date1.Date & "' AND Date <= '" & date2.Date & "'"
            Dim dataread3 As SqlDataReader = cmd.ExecuteReader
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        'ElseIf cbYearLevel.Text = "" And cbCourse.Text = "" Then
        '    SortDate3()
    ...
    Public Function DateCHECKER()
        Try
            ...
            Dim query As String = "SELECT * FROM tblAttendance WHERE Date = @DAT"
    ...
    Sub Search()
        Try
            ...
            sql = "Select * from tblAttendance where StudentID Like '%" & txtStudentID.Text & "%' AND YearLevel Like '%" & cbYearLevel.Text & "%' AND Course Like '%" & cbCourse.Text & "%'AND Department Like '%" & cbDepartment.Text & "%'"
